...
function main() {
    ...
    pushCheck(results, 'deleteService', 'delete-service 使用行级删除计划并等待 deleteTableRowsBatch()', has(contents.deleteService, 'deleteTableRowsBatch') && has(contents.deleteService, 'async function executeTheaterDeletionPlans(scene, plans = [])') && has(contents.deleteService, 'const result = await deleteTableRowsBatch(plan.tableName, plan.rowIndexes);') && !has(contents.deleteService, 'refreshProjection'));
