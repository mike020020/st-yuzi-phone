...
本文在玉子美化工作台中是全局视觉参照，不是可直接调用的运行时依赖。文中指向的 `styles/phone-base/00-phone-tokens.css`、`modules/phone-core/navigation-ui.js` 和小手机资源属于上游手机工程，可能不在本独立目录中；玉子美化页面不得导入、读取或修改它们。

制作 Bundle 时应从本文读取需要的尺寸、热区、图标形状和视觉语义，在当前 `projects/<id>/` 内建立页面自己的作用域变量；需要图标或图片时，将资源复制为当前项目内的登记资源。这样视觉与小手机保持一致，同时继续遵守玉子美化不依赖父项目 Runtime、源码或 `node_modules` 的边界。
